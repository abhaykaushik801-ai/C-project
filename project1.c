 ...
 *
 * Compile:
 *   Linux/macOS: gcc -std=c99 -O2 -o snake_game_noflicker snake_game_noflicker.c
 *   Windows (MinGW): gcc -std=c99 -O2 -o snake_game_noflicker.exe snake_game_noflicker.c
 *
 * Author: ChatGPT
 */
...
#include <time.h>
  ...
  /* Fallback: older MinGW or Windows SDK headers may not define this constant.
     Define it ourselves so compilation succeeds. */
...
/* Enable raw mode for POSIX (non-blocking, no-echo) */
...
/* Try to enable ANSI VT processing on Windows so escape sequences work */
...
/* ---------------- Rendering WITHOUT flicker ----------------
   Compose the entire frame in memory and write once. Hide cursor while updating.
*/
...
int main(void) {
    ...
    /* Move cursor below board and print results */
